...
public class CassandraToArrowUtils {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CassandraToArrowUtils.class);
    ...
    public static ArrowType getArrowTypeForCassandraField(CassandraFieldInfo fieldInfo, Calendar calendar) {
        ...
        if(fieldInfo.getDataType() != null){
            ...
            if (type.getTypeID().equals(ArrowType.Null.INSTANCE.getTypeID())) {
                LOGGER.warn("getSchema: returned %s as type for column[%s]", ArrowType.Null.INSTANCE, fieldInfo.getName());
            }
            ...
            LOGGER.warn(String.format("getSchema: Unable to map type for column[%s].  'type' was null in schema.", fieldInfo.getName()));
